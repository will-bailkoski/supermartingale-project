...
def verify_model_gurobi(n, h, equil_set, C, B, r, epsilon, W1, W2, B1, B2):
    ...
    x_tplus1_up = model.addVars(n, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="upx")
    x_tplus1_down = model.addVars(n, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="downx")
    ...
    for j in range(h):
        model.addConstr(hidden_layer_Px_up[j] == gp.quicksum(W1[j][i] * x[i] + B1[j] for i in range(n)))
        model.addGenConstrMax(relu_layer_Px_up[j], [hidden_layer_Px_up[j], 0])
        model.addConstr(hidden_layer_Px_down[j] == gp.quicksum(W1[j][i] * x[i] + B1[j] for i in range(n)))
        model.addGenConstrMax(relu_layer_Px_down[j], [hidden_layer_Px_down[j], 0])

    model.addConstr(V_Px_up == gp.quicksum(W2[0][i] * relu_layer_Px_up[i] + B2[0] for i in range(h)))
    model.addConstr(V_Px_down == gp.quicksum(W2[0][i] * relu_layer_Px_down[i] + B2[0] for i in range(h)))
    ...
    if model.status == GRB.OPTIMAL:
        ...
        print([P_x[i].X for i in range(n)])
        print([x_tplus1_up[i].X for i in range(n)])
        print([x_tplus1_down[i].X for i in range(n)])
